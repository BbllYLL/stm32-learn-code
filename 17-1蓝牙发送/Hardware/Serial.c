...
void BL_Text_State(void)
{
	...
	if(USART_GetFlagStatus(USART1, USART_FLAG_RXNE) == SET)
	{
		...
		else if(RxState == 1)
		{
			...
			{
				RxState = 0;
				Serial_RxString[pRxState] = '\0';
				Serial_RxFlag = 1;
			}
			else
			{
				Serial_RxString[pRxState] = RxData;
				pRxState ++;			
			}
		}
	}
...
void BL_LED_Turn(void)
{
	...
	{		
		...
		if(strcmp(Serial_RxString, "LED_Turn") == 0)
		{
			LED1_Turn();
			if(GPIO_ReadOutputDataBit(GPIOA, GPIO_Pin_1) == RESET)
			{
				OLED_ShowString(4,1,"                ");
				OLED_ShowString(4,1,"LED1_ON_OK");
